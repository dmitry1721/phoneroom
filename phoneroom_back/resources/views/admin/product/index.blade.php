                                            ...
                                            <div class="card--content mr-5">
                                                <div class="row" style="flex-direction: row-reverse;">
                                                    ...
                                                    <div class="col">
                                                        ...
                                                        <p class="">Категория - {{  $product->category->name  }}</p>
                                                        <p class="">Бренд - {{  $product->brand->name  }}</p>
                                                        <p class="">Цена: {{  json_decode($variant->variants_json, true)['price']  }} р.</p>
                                                        <b><a href="{{ route('admin.products.variant_show', [$product->slug, json_decode($variant->variants_json, true)['slug']]) }}">Подробнее</a></b>
                                                        <form class="mt-5" action="{{ route('admin.products.variant_destroy', [$product->slug, json_decode($variant->variants_json, true)['slug']]) }}" method="post">
                                                            @csrf
                                                            @method('delete')
...
{{--                                                    <a class="d-inline-block " href="{{ route('admin.products.edit', $product->slug) }}"><i class="fa fa-solid fa-pen"></i></a>--}}
                                                ...
                                                <div class="col">
                                                    ...
                                                    <b><a href="{{ route('admin.products.show', $product->slug) }}">Подробнее</a></b>
                                                    <form class="mt-5" action="{{ route('admin.products.destroy', $product->slug) }}" method="post">
                                                        @csrf
                                                        @method('delete')
                                                        <button class="btn btn-danger" type="submit">Удалить</button>
                                                    </form>
