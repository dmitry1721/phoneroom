...
//            })
    ...
    public function show(Category $category){
        ...
        $min_price = 9999999;
        $max_price = -1;
        $min_price_main = 9999999;
        $max_price_main = -1;
        if ($category_section['parent_id'] === null){
            foreach ($category_section['subCategories'] as $sub_category){
                if ($this->show(Category::where('slug', $sub_category->slug)->first())['min_price'] < $min_price_main){
                    $min_price_main = $this->show(Category::where('slug', $sub_category->slug)->first())['min_price'];
                }
                if ($this->show(Category::where('slug', $sub_category->slug)->first())['max_price'] >= $max_price_main){
                    $max_price_main = $this->show(Category::where('slug', $sub_category->slug)->first())['max_price'];
                }
            }
        }
        else{
        ...
        $category_section += [
            ...
            'max_price_main' => (int)$max_price_main,
            'min_price_main' => (int)$min_price_main,
        ];
