        ...
        if (count($category->products)){
            if(count($category->load('variants')->variants)){
                ...
                foreach ($category_variants_json as $variant_price_desc){
                    ...
                    $product = Product::where('name', trim(stristr(json_decode($variant_price_desc, true)['product_name'], json_decode($variant_price_desc, true)['name'], true)))->first();
                    $variant_price_desc = json_decode($variant_price_desc, true);
                    $variant_price_desc['properties'] = json_decode($product->property->properties_json, true);
